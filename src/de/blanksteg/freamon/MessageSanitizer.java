...
/**
 * The MessageSanitizer is supposed to deal with the various idiosyncrasies inherent in online communication and random
 * response generation. Its main functions are to make incoming messages more usable for statistical analysis and
 * outgoing messages more pleasing to the eye.
 * 
 * Generally, any URL is worthless for a statistical chatbot, because any mutation in a URL will likely result in an
 * invalid one. Hence, URLs are filtered from incoming messages. Certain special characters used in online communication
 * such as < and @ tend to spoil generated messages with seemingly random usage of said characters, so these are
 * filtered as well. Additionally, any leading !word segments of a message are deleted because these are mainly chat
 * commands. Leading and trailing whitespace will be trimmed.
 * 
 * When considering outgoing messages, this class mainly ensures capitalization at the beginning of the response,
 * removing redundant punctuation and spaces, and ensuring some common sentence delimiter.
 * 
 * @author Marc Müller
 */
