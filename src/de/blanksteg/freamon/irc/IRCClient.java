...
/**
 * The IRCClient class offers semi-advanced functionality to manage connections to multiple IRC
 * networks in which multiple channels can be joined either as an active or passive member. When a
 * channel is joined or another user joins a channel the IRCClient queries a stored instance of an
 * {@link GreetingsGenerator} to possibly generate a greeting appropriate for the event. Similarly,
 * it checks a {@link ResponseGenerator} for received messages and, if possible, responds using them.
 * 
 * Network management is done using {@link Network} instances that are collected in this class. Networks
 * can be added and removed after the IRCClient instance has been created. If the client is already
 * connected and a network is added / removed, the client will connect / disconnect accordingly.
 * 
 * For additional monitoring, classes implementing {@link Listener} can register themselves as
 * subscribers to this client. Subscribers will be registered as event listeners to every network
...
public class IRCClient extends ListenerAdapter<Network>
{
   ...
   * @throws IOException
   * @throws IrcException
   * @throws UnsupportedOperationException If the client is already connected.
   */
   ...
   * 
   * @throws UnsupportedOperationException If the client is not connected.
   */
  ...
  /**
   * Adds a new IRC network to be managed by this client. The network is only added if it is not
   * known by this client. If the network is added and this client is considered connected, it will
   * automatically connect to the network.
   * 
   * @param newNetwork The network to manage.
   * @return Whether or not the network was added.
   * @throws IOException
   * @throws IrcException
   * @throws IllegalArgumentException If the network is null or doesn't have a host set.
   */
  ...
  /**
   * Removes an IRC network managed by this client. The network is only removed if it is known by
   * this client. If the network is removed and this client is considered connected, it will
   * automatically disconnect from the network.
   * 
   * @param newNetwork The network to remove's host.
   * @return Whether or not the network was removed.
   * @throws IOException
   * @throws IrcException
   * @throws IllegalArgumentException If the network host is null or empty
   */
   ...
   * automatically disconnect from the network.
   * 
   * @param newNetwork The network to remove
   * @return Whether or not the network was removed.
   * @throws IOException
   * @throws IrcException
   * @throws IllegalArgumentException If the network host is null
   */
  ...
  public synchronized void onPing(PingEvent<Network> event)
  {
    ...
    target.sendMessage(source, "I'm a brainless grunt -- don't trust me! My serial number is: " + Math.abs(source.hashCode()));
  }
