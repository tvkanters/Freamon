...
/**
 * A network stores the basic information needed to connect to an IRC network. Additionally, it
 * maintains two sets of active and passive channels. Active channels are ones the bot is allowed
 * to talk in, while passive channels are ones he can only learn from. The distinction between them
 * makes little difference to this class and is mainly used by the {@link IRCClient}.
 * 
 * @author Marc Müller
 */
...
>>>>>>> 6bf20edb08ffc49bae4d3ea6e73cfb25e4ef5972
   * 
   * @param newUrl The host to connect to.
   * @param newPort Target port to connect on.
   * @param newNickNames Array of nickname preferences.
   * @param newUserName The supposed user name on the network.
   * @param newRealName The supposed real name on the network.
   * @param newClientName The supposed client's name.
   * @throws IllegalArgumentException If either of these values is null or the port is outside of legal range.
   */
  public Network(String newUrl, int newPort, String[] newNickNames, String newUserName, String newRealName, String newClientName, String newServerPass)
  {
    if (newUrl == null || newNickNames == null || newUserName == null || newRealName == null || newClientName == null)
    {
      throw new IllegalArgumentException("Null was passed as a property to the network: (host = " + newUrl +", nickNames = " + newNickNames + ", user name = " + newUserName +", real name = " + newRealName + ", client = " + newClientName +")"); 
    }
   ...
   * @throws IllegalArgumentException If either of these values is null or the port is outside of legal range.
   */
  public Network(String newUrl, int newPort, String[] newNickNames, String newUserName, String newRealName, String newClientName)
  {
    this(newUrl, newPort, newNickNames, newUserName, newRealName, newClientName, Configuration.DEFAULT_PASS);
  }
  
  /**
   * Adds a channel to actively participate in to this network. If the channel is already joined
   * either actively or passively, this request is ignored. If we are already connected the channel
   * is entered immediately, otherwise it will be remembered for when we do connect.
   * 
   * @param toAdd The channel's name.
   * @return Whether or not the channel was added to the set of active channels.
   * @throws IllegalArgumentException If the channel name is null, shorter than two or does not match {@link Configuration#CHANNEL_MATCH}. 
   */
  ...
  /**
   * Removes a channel from the set of channels to be active in. If we were never in that channel,
   * nothing is done. If we are connected to the network, the client parts from the channel with
   * {@link Network#quitMessage} as the reason.
   * 
   * @param channelName The name of the channel to remove.
   * @return True iff the channel was removed.
   * @throws IllegalArgumentException If null is passed as the channel name.
   */
  ...
  /**
   * Adds a channel to lurk in to this network. If the channel is already joined either actively or
   * passively, this request is ignored. If we are already connected the channel is entered
   * immediately, otherwise it will be remembered for when we do connect.
   * 
  ...
  /**
   * Removes a channel from the set of channels to be passive in. If we were never in that channel,
   * nothing is done. If we are connected to the network, the client parts from the channel with
   * {@link Network#quitMessage} as the reason.
   * 
   * @param channelName The name of the channel to remove.
   ...
   * @throws IOException
   * @throws IrcException
   * @throws UnsupportedOperationException When we already are connected.
   */
   ...
   * 
   * @param channelName The channel to become passive in.
   * @return Whether or not we switched to be passive.
   * @throws IllegalArgumentException If the given channel name is null.
   * @throws UnsupportedOperationException If we are not even in the given channel. 
   */
   ...
   * 
   * @param channelName The channel to become active in.
   ...
   * @throws IllegalArgumentException If the channel name is null.
   */
  ...
  /**
   * Leave the channel by the given name regardless of whether we're active or passive in it. The
   * channel is removed from its respective set.
   * 
   * @param channel The channel to leave.
   * @throws IllegalArgumentException If the given channel name is null.
   * @throws IllegalStateException If we are not in the channel.
   */
  ...
  /**
   * Leaves the given channel. This is only a small helper method needed to resolve a channel's name
   * to it's {@link Channel} instance needed for {@link PircBotX#partChannel(Channel, String)}. The
   * part message will be {@link Network#quitMessage}.
   * 
   * @param channelName
   */
   ...
   * 
   * @param ignored The message source to be ignored.
   */
   ...
   * 
   * @param ignored The message source to check.
